...
## Changing to standard scores after Yang re-read source article - 4/18/25 (prepended "scale" to below specifications)

combo$cell.sma <- scale(with(combo, NS.ma/NS)*100)
combo$cell.smb <- scale(with(combo, NS.mb/NS)*100)
combo$cell.sfa <- scale(with(combo, NS.fa/NS)*100)
combo$cell.sfb <- scale(with(combo, NS.fb/NS)*100)
combo$cell.pma <- scale(with(combo, PPmale*PPdepta)*100)
combo$cell.pmb <- scale(with(combo, PPmale*PPdeptb)*100)
combo$cell.pfa <- scale(with(combo, PPfemale*PPdepta)*100)
combo$cell.pfb <- scale(with(combo, PPfemale*PPdeptb)*100)
...
*Research question 3*: What impact does the application of weights have on both biased and unbiased sample estimates? 
...
```{r example1, fig.cap="Visual representation of terms used to describe population elements as well as (N's)."}
...
[^sample]: Throughout the Method and Results, "lowercase" specification of simulation groups indicates sample constituencies (e.g., male.b) whereas uppercase implicates population (e.g., Male.B).
...
```{r Cattell3, echo=FALSE, fig.cap="Effect of subgroup sampling rate match with population constituency on point estimate  misrepresentation.",fig.height=4.5, fig.width=6.5}

ggplot(combo, aes(x=r.p, y=PPSU)) + 
  geom_point(alpha=1/25,size=1.5) + 
  xlim(.8, 1.0) + ## changed 4/18 (-1.0, 1.0)
  ylim(0, .3) + 
...
```{r CattelExplain, fig.cap="Allocation of response rates relative to underlying distributional form and consequential association with population misrepresentation"}
